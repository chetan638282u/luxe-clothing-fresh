...
import { menProducts as products } from '../../data/catalog'
import useMediaQuery from '../../hooks/useMediaQuery'

const winter = products.filter(p => p.season === 'winter')
const summer = products.filter(p => p.season === 'summer')
...
export default function MenCollection({ onClose, hash }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={isMobile ? { display: 'none', opacity: 0, transition: { duration: 0.15 } } : { opacity: 0, transition: { duration: 0.05 } }}
      ...
      <div className="min-h-full flex flex-col px-6">
        <div className="max-w-7xl mx-auto w-full pt-12 md:pt-16 flex-1 flex flex-col">
          <div className="flex items-center justify-between mb-8 md:mb-12">
            ...
            <h2 className="font-heading text-3xl md:text-4xl text-ivory">Men's Collection</h2>
            <div className="w-16" />
          </div>
